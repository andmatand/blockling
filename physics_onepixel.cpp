...
void telepad::Teleport() {
	...
	// DEBUG: view sourceSurf
	SDL_SaveBMP(sourceSurf, "sourceSurf.bmp");
	...
	bool map[sH * TILE_W]; // Keeps track of which pixels have been teleported (false = hasn't been moved, true = has)
	// Mark transparent pixels as already moved, all others as not yet moved
	for (pY = 0; pY < sH; pY++) {
		for (pX = 0; pX < TILE_W; pX++) {
			col = GetPixel(sourceSurf, pX, pY);
			if (col == SDL_MapRGB(sourceSurf->format, 0xff, 0x00, 0xff)) {
				map[(pY * TILE_W) + pX] = true;
			}
			else {
				map[(pY * TILE_W) + pX] = false;
			}
		}
	}
	...
	while (true) {
		...
		// Move multiple pixels at a time, and always proportionate to the
		// load size (so it will always appear to be the same speed)
		for (uint i = 0; i < sH; i++) {
			// Check if we've moved all the pixels over
			numPixels = 0;
			for (int j = 0; j < sH * TILE_W; j++) {
				if (map[j] == true) numPixels ++;
			}
			if (numPixels == sH * TILE_W) break;
		
			while (true) {
				pX = rand() % TILE_W;
				pY = rand() % sH;
				
				// If this pixel has not been moved yet.
				if (map[(pY * TILE_W) + pX] == false) {
					map[(pY * TILE_W) + pX] = true;
			...
			PutPixel(destSurf, pX, pY, col);

			// Erase source pixel (with transparent color)
			PutPixel(sourceSurf, pX, pY, SDL_MapRGB(sourceSurf->format, 0xff, 0x00, 0xff));
		}
